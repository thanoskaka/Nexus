...
function PathCard({ path, onLaunch }: { path: PathDef; onLaunch: () => void }) {
  const isCTA = path.link.onClick !== undefined;
  return (
...
export function PublicHome({ authError, onLaunch, signedOut }: PublicHomeProps) {
  const pathsWithCTA: PathDef[] = PATHS.map((p) =>
        ...
        <section className="container mx-auto px-4 py-10">
          <div className="mx-auto max-w-5xl">
            <div className="grid gap-4 sm:grid-cols-3">
              {pathsWithCTA.map((path, idx) => (
                <div key={idx} className="contents">
                  <PathCard path={path} onLaunch={onLaunch} />
                </div>
              ))}
            </div>
          </div>
