...
function keyFor(providerId: ProviderCredentialId): string { return KEY_PREFIX + providerId; }
function prefFor(providerId: ProviderCredentialId): string { return PREF_PREFIX + providerId; }
...
export function saveLocalCredential(providerId: ProviderCredentialId, apiKey: string): LocalCredentialEntry {
  ...
  const entry: LocalCredentialEntry = { key: trimmed, last4: trimmed.slice(-4), updatedAt: Date.now() };
  localStorage.setItem(keyFor(providerId), JSON.stringify(entry));
  return entry;
}

export function removeLocalCredential(providerId: ProviderCredentialId): void { localStorage.removeItem(keyFor(providerId)); }

export function getLocalPreference(providerId: ProviderCredentialId): ProviderPreference {
  try { const raw = localStorage.getItem(prefFor(providerId)); if (raw === 'hosted' || raw === 'user') return raw; } catch {}
  return 'hosted';
}
