...
export function getSampleMode(): boolean {
  ...
  try {
    cachedSampleMode = window.localStorage.getItem(SAMPLE_MODE_KEY) === 'true';
  } catch {
    cachedSampleMode = false;
  }
...
export function setSampleMode(active: boolean) {
  ...
  try {
    if (active) {
      window.localStorage.setItem(SAMPLE_MODE_KEY, 'true');
    } else {
      window.localStorage.removeItem(SAMPLE_MODE_KEY);
    }
  } catch {}
}
...
export function SampleModeProvider({ children }: { children: React.ReactNode }) {
  ...
  const enableSampleMode = useCallback(() => {
    setSampleMode(true);
    setIsActive(true);
  }, []);

  const disableSampleMode = useCallback(() => {
    setSampleMode(false);
    setIsActive(false);
  }, []);

  const value = useMemo<SampleModeContextType>(
    () => ({
      isSampleMode: isActive,
      enableSampleMode,
      disableSampleMode,
      sampleData: SAMPLE_PORTFOLIO_DATA,
    }),
    [isActive, enableSampleMode, disableSampleMode],
  );
...
export function useSampleMode(): SampleModeContextType {
  ...
  if (!ctx) {
    return {
      isSampleMode: getSampleMode(),
      enableSampleMode: () => setSampleMode(true),
      disableSampleMode: () => setSampleMode(false),
      sampleData: SAMPLE_PORTFOLIO_DATA,
    };
  }
  return ctx;
